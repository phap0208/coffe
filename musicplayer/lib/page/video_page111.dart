...
class VideoPlayerPage111 extends StatefulWidget {
  const VideoPlayerPage111({Key? key}) : super(key: key);

  @override
  State<VideoPlayerPage111> createState() => _VideoPlayerPage111State();

}

class _VideoPlayerPage111State extends State<VideoPlayerPage111> {
  ...
  Future<void> _createRoomAndShowCode() async {
    try {
      final response = await _dio.get('https://us-central1-ikara-development.cloudfunctions.net/ktv1_createRoom-createRoom');
      final roomId = response.data['roomId'];
      _showRoomCode(roomId);
    } catch (e) {
      print('Error: $e');
    }
  }

  void _showRoomCode(String roomId) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Mã phòng của bạn:'),
          content: Text(roomId),
          actions: [
            TextButton(
              onPressed: () {
